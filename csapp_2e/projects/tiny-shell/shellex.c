...
void reap_child(int signo) // 相应子进程的结束信号的handler
{
        for (;;) {
...
void wait_for_job(pid_t pgid) // 用于等待某个前台进程
{
        int status;
...
int builtin_command(char **argv)
{
        if (!strcmp(argv[0], "quit")) {
                ...
                kill(-pgid, SIGCONT); // 重启这个进程
                if (!strcmp(argv[0], "fg")) { // 前台重启
...
void eval(char *cmdline) {
        ...
        if (builtin_command(argv)) // 如果是内置的命令quit，那么就在这个函数内部就直接执行了
                return;
        ...
        buf2[len2] = '\0';
        if (!bg) {
                set_foreground(pid, buf2, len2);    // 如果是前台进程，那么等待这个前台进程
                wait_for_job(pid);
        } else {
                printf("%d %s", pid, cmdline);      // 如果是后台进程，那么不等待
                // pid_t ret = waitpid(pid, NULL, WNOHANG | WUNTRACED);
                // if (ret == 0) // 注意这个地方不是常规的，不发生阻塞。
...
void stop_handler(int signo)            // shell接收到信号，重新发送给前台进程，将前台进程标记为后台进程 && 停止状态
{
...
void interrupt_handler(int signo) // shell接收到信号，重新发送给前台进程组
{
        pid_t pgid = foreground_pgid();// 获取前台进程组中的组号
